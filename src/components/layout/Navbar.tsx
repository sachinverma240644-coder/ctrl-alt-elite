...
import { Building2, Home, PlusCircle, CreditCard, Box, User, Settings, PieChart } from 'lucide-react';
...
export function Navbar({ role }: { role: 'student' | 'admin' }) {
  ...
  const adminLinks = [
    ...
    { name: 'Rent', href: '/admin/rent', icon: PieChart },
  ];
  ...
  return (
    <>
      ...
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center">
        {/* Unified View with Desktop & Mobile styling handled by tailwind */}
        <div className="flex h-16 liquid-glass rounded-full md:rounded-full px-4 items-center gap-1 min-w-[320px] md:min-w-[500px] border border-white/20 shadow-2xl">
            ...
            {links.map((link) => (
              <Link key={link.href} href={link.href}>
                <Button
                  variant="ghost"
                  className={cn(
                    "rounded-xl md:rounded-full px-3 md:px-6 h-11 transition-all duration-300 relative group flex flex-col md:flex-row items-center",
                    pathname === link.href 
                ...
                >
                  ...
                  <span className="text-[10px] md:text-sm font-semibold uppercase md:capitalize">{link.name}</span>
                  {pathname === link.href && (
                    <span className="hidden md:block absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-white rounded-full" />
                  )}
                </Button>
              </Link>
            ))}
